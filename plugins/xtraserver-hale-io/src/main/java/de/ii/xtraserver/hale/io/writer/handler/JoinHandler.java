...
class JoinHandler extends AbstractTypeTransformationHandler {
  ...
  public void doHandle(
    ...
    boolean isInnerJoin =
        typeCell.getTransformationParameters().get("innerJoin").stream()
            .map(ParameterValue::getStringRepresentation)
            .map(Boolean::parseBoolean)
            .findFirst()
            .orElse(false);

    for (final ParameterValue transParam :
        typeCell.getTransformationParameters().get(PARAMETER_JOIN)) {
      ...
      final MappingTableBuilder baseTable =
          createTableIfAbsent(joinParameter.getTypes().iterator().next());

      final List<Condition> sortedConditions =
      ...
      while (!sourceTables.isEmpty()) {
        ...
        for (String tableName : sourceTables) {
              ...
              sortedConditions.stream()
                  ...
                  .map(
                      condition -> {
                        ...
                        if (isInnerJoin) {
                          join.transformationHint(HINT_JOIN_TYPE, "INNER");
                        }
                        return join.build();
                      })
          ...
          joins.forEach(
              joinPath -> {
                mappingContext
                    .getTable(joinPath.getTargetTable())
                    .ifPresent(targetTable -> targetTable.joinPath(joinPath));
              });

          nextSourceTables.addAll(
              joins.stream().map(st -> st.getTargetTable()).collect(Collectors.toList()));
        }
  ...
  /**
   * Transform and sort hale Join Condition Set into Join Conditions that are attached to multiple
   * XtraServer Join objects
   *
   * @param joinParameter hale conditions
   * @param sourceTypes source types from cell with primary keys
   * @return sorted joins
   */
  private List<Condition> transformSortedConditions(
      final JoinParameter joinParameter, final Collection<? extends Entity> sourceTypes) {

    return joinParameter.getConditions().stream()
        .sorted(
            new Comparator<JoinCondition>() {
        ...
        .map(
            condition -> {
              final TypeEntityDefinition baseType =
                  AlignmentUtil.getTypeEntity(condition.baseProperty);

              final TypeEntityDefinition joinType =
                  sourceTypes.stream()
                      .filter(
                          entity ->
                              entity
                      ...
                      .map(entity -> AlignmentUtil.getTypeEntity(entity.getDefinition()))
                      .findFirst()
              ...
              final MappingJoin.Condition mappingJoinCondition =
                  new MappingJoinBuilder.ConditionBuilder()
                      .sourceTable(baseTable.buildDraft().getName())
                      .sourceField(baseField)
                      .targetTable(joinTable.buildDraft().getName())
