...
class JoinHandler extends AbstractTypeTransformationHandler {
  ...
  public void doHandle(
    ...
    List<ParameterValue> typeCellJoinParameters =
    ...
    for (final ParameterValue transParam : typeCellJoinParameters) {
      ...
      final List<JoinParameter.JoinCondition> sortedConditions =
          transformSortedConditions(joinParameter);
      for (JoinParameter.JoinCondition jc : sortedConditions) {
        final TypeEntityDefinition baseType = AlignmentUtil.getTypeEntity(jc.baseProperty);
                ...
                .filter(
                    entity ->
                        entity
                            ...
                            .getType()
                            .getName()
                            .equals(jc.joinProperty.getType().getName()))
                .map(entity -> AlignmentUtil.getTypeEntity(entity.getDefinition()))
                .findFirst()
                .orElse(AlignmentUtil.getTypeEntity(jc.joinProperty));

        String baseTableName = baseType.getType().getDisplayName();
        final MappingTableBuilder baseTable = createTableIfAbsent(baseType);
        final String baseField =
            jc.baseProperty.getPropertyPath().iterator().next().getChild().getDisplayName();
        String joinTableName = joinType.getType().getDisplayName();
        final MappingTableBuilder joinTable = createTableIfAbsent(joinType);
        final String joinField =
            jc.joinProperty.getPropertyPath().iterator().next().getChild().getDisplayName();

        String baseTableJoinField =
            jc.baseProperty.getLastPathElement().getChild().getName().getLocalPart();
        String joinTableJoinField =
            jc.joinProperty.getLastPathElement().getChild().getName().getLocalPart();

        JoinInfo ji = new JoinInfo(baseTableName, baseField, joinTableName, joinField);
  ...
  private List<JoinParameter.JoinCondition> transformSortedConditions(
      final JoinParameter joinParameter) {
