...
public class XtraServerMappingUtils {
  ...
  /**
   * Return the first source {@link Entity}, which is assumed to be a {@link Property}.
   *
  ...
  /**
   * Return all source {@link Entity} which are a {@link Property}.
   *
   * @param propertyCell the property cell
   * @return the target map of {@link Property}
   */
  public static Map<String, Property> getSourceProperties(Cell propertyCell) {
    ListMultimap<String, ? extends Entity> sourceEntities = propertyCell.getSource();
    if (sourceEntities != null && !sourceEntities.isEmpty()) {
      return sourceEntities.entries().stream()
          .filter(e -> e.getValue() instanceof Property)
          .map(e -> Map.entry(e.getKey(), (Property) e.getValue()))
  ...
  /**
   * Return the first target {@link Entity}, which is assumed to be a {@link Property}.
   *
   * @param propertyCell the property cell
   * @return the target {@link Property}
   */
